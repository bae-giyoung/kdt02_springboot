...
public class OAuth2SuccessHandler extends SimpleUrlAuthenticationSuccessHandler {
	...
	Map<String, String> getUserInfo(Authentication authentication) {
		...
		if(provider.equalsIgnoreCase("naver")) {
			...
			email = (String)((Map<String, Object>)user.getAttributes().get("kakao_account")).get("email");
			
		} else if(provider.equalsIgnoreCase("github")) {	
			email = (String)((Map<String, Object>)user.getAttributes()).get("email");
			if(email == null) email = (String)(((Map<String, Object>)user.getAttributes()).get("id")).toString();
			
		} else {
			// google, meta(facebook)
			//System.out.println("provider: "+provider+", user: "+user+", email: "+email);
